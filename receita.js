/* RefilMed — documentos (receita simples, controle especial em 2 vias, orientações)
...
(function (G) {
  ...
  function pacienteBloco(p, modo) {
    ...
    if (modo === 'cpf' || modo === 'tudo') s += `<br><b>CPF:</b> ${esc(RF.fmtCPF(pa.cpf) || '—')}`;
  ...
  function assinatura(med, ass, data) {
    ...
    let s = `<div style="margin-top:34px;font-family:var(--font);font-size:12.5px">${esc(cidade ? cidade + ', ' : '')}${esc(RF.fmtData(data || RF.agora(), true))} · Documento emitido em telemedicina</div>`;
    s += `<div class="ass">${esc(med.nome || '')}<br>CRM ${esc(med.crm || '')}/${esc(med.uf || '')}${med.rqe ? ' · RQE ' + esc(med.rqe) : ''}`;
    if (ass) s += `<br><span style="font-size:11px">${ass.tipo === 'icp' ? 'Original assinado digitalmente (ICP-Brasil) no PDF do pedido' : 'DEMONSTRAÇÃO: sem validade legal'} · ${esc(RF.fmtData(ass.em, true))}</span>`;
    return s + '</div>';
  }
  ...
  function itensHTML(bloco) {
    return '<ol>' + bloco.itens.map(it => `<li><span class="n">${esc(it.nome)}</span>${it.forma ? ', ' + esc(it.forma) : ''}<br>
      ${esc(it.posologia || '')}<br><span style="font-size:12.5px">Quantidade: ${esc(RF.textoQuantidade(it, bloco.tipo))}${it.quantidade != null ? ' · ' + it.dias + ' dias de tratamento' : ''}${it.obs ? ' · ' + esc(it.obs) : ''}</span></li>`).join('') + '</ol>';
  }

  // devolve HTML das folhas; opts: {ass, orientacoes:boolean}
  RF.folhasReceita = function (p, med, opts) {
    opts = opts || {};
    ...
    for (const b of p.receitas || []) {
      ...
      if (b.tipo === 'simples') {
        ...
        for (const via of vias) {
          folhas.push(`<div class="folha"><div class="via">${via}${b.numeroSNCR ? ' · Numeração SNCR ' + esc(b.numeroSNCR) : ''}</div>${cabecalho(med)}<div class="tit">RECEITUÁRIO DE CONTROLE ESPECIAL</div>${pacienteBloco(p, 'cpf')}<div class="uso">USO ORAL</div>${itensHTML(b)}${justificativas(b)}${assinatura(med, ass, data)}
    ...
    if (opts.orientacoes) {
      ...
      const corpo = meds.map(m => {
        ...
        if (!k) return `<h4>${esc(RF.nomeRx(m))}</h4><p>Siga a orientação do médico.</p>`;
        return `<h4>${esc(RF.nomeRx(m))}</h4><ul>${(k.efeitosComuns || []).slice(0, 4).map(x => '<li>Pode causar: ' + esc(x) + '</li>').join('')}${(k.sinaisAlerta || []).slice(0, 3).map(x => '<li>Procure atendimento se: ' + esc(x) + '</li>').join('')}${k.suspensao ? '<li>' + esc(k.suspensao) + '</li>' : ''}</ul>`;
      }).join('');
  ...
  RF.conteudoAssinado = function (p, med, em) {
  ...
  RF.pdfTemAssinatura = function (bytes) {
